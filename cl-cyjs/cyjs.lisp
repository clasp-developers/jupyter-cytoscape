...
(defmethod from-igraph ((self cyjs) igraph-network &key (layout nil) (scale 100.0))
  (list (cons "elements" (list (cons "nodes" (vector (list (cons "data" (list (cons "id" 0)
                                                                              (cons "name" "R1")
                                                                              (cons "type" "foo")))
                                                           (cons "data" (list (cons "id" 1)
                                                                              (cons "name" "R2")
                                                                              (cons "type" "bar"))))))
                               (cons "edges" (vector (list (cons "data" (list (cons "source" 0)
                                                                              (cons "target" 1)
                                                                              (cons "edgeType" "baz"))))))))
        (cons "data" (list))))
...
(defmethod msg-arrived ((self cyjs) name new old)
  (when (slot-boundp self 'status)
    (setf (status self)
